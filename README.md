...
1. **Setup (once per organization/desk):** drop a handful of your own past CAMs into
   `inputs/calibration_samples/` and run `scripts/calibrate.py`. This extracts your writing
   tone, structure, and standard risk phrasing into `config/style_guide.md`, which every
   subsequent draft is written against.
...
If a deal's `--type` doesn't match an existing template under `templates/`, the orchestrator
treats it as a genuinely new CAM type and saves the drafted structure as a new template — so
your template library grows to match the kinds of deals you actually do, instead of forcing
every deal through one fixed layout.
...
- [`templates/corporate_credit_cam.md`](templates/corporate_credit_cam.md) — general corporate
  lending (RCF/term loan/overdraft): facility terms, covenants, security package, full financial
  spreading, industry/competitive analysis, risks & mitigants.
- [`templates/asset_finance_cam.md`](templates/asset_finance_cam.md) — asset-backed / equipment
  or fleet finance: adds per-asset LGD/RV/collateral-cover tables and asset-specific facility
  conditions (max asset age, sublet terms, residual value treatment) on top of the same
  financial-analysis and risk sections.

Both are plain Markdown with bracketed `[placeholder]` fields — safe to fork and edit directly,
and readable as a diff in git. New deal types are added the same way (manually, or
auto-generated by the orchestrator the first time that `--type` is used).
...
> template, that's on the roadmap to support directly (see below) — for now,
> `spreading_builder.py`'s layout is the default format.
...
2. **Set your API key:**
   ...
   ```
3. **(Recommended) Calibrate to your own style:** put a few of your own historical CAMs (PDF)
   into `inputs/calibration_samples/`, then run:
   ```bash
   python scripts/calibrate.py
   ```
   This writes `config/style_guide.md`. Skip this step to use a neutral default tone.
4. **Run a deal:**
   ```bash
...
- [ ] First-run guided setup (interactive prompt to add calibration samples, rather than a
      manual file drop) that also derives a base CAM template shape from the samples, not just
      tone/style.
