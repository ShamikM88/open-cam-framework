...
├── config/
│   ├── settings.json            Model name, token limit, default currency, I/O directories
...
- **`scripts/orchestrator.py`** — the main pipeline. For a given `--company`, `--proposal`,
  ...
  Underwriter draft, runs the Risk Reviewer audit, auto-saves a new template under `templates/`
  if that deal type hasn't been seen before, and exports `.docx`/`.xlsx` into
